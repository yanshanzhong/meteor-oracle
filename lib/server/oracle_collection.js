...
OracleCollection.prototype.find = function(selector, fields, options) {
	...
	if(self.oracleOptions.sqlAddId) {
		sql = "SELECT rownum as \"_id\", c.* FROM ("+sql+") c";
	}
	...
	if(sqlTable) {
		...
		for(var di in tableDesc.details) {
			...
			var detSql = "SELECT * FROM "+detTableName+" WHERE \"_id\" IN (SELECT \"_id\" FROM ("+sql0+"))";
			...
			if(sorter && sorter[detRec.fieldName] && sorter[detRec.fieldName]["."]) {
				detSql = detSql + " ORDER BY " + sorter[detRec.fieldName]["."] + ", \"_id\", \"_indexNo\"";
			} else {
				detSql = detSql + " ORDER BY \"_id\", \"_indexNo\"";
			}
...
OracleCollection.prototype.ensureIndex = function(fields, options, callback) {
	...
	if(cl["."]) {
		...
		if(s) {
		var sql = "CREATE "+(options.bitmap?"BITMAP ":"")+(options.unique?"UNIQUE ":"")+"INDEX \""+sqlTable+"I1\" ON \""+sqlTable+"\"("+s+")";
		var sqlParameters = [];
