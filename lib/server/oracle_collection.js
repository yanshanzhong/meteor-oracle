...
OracleCollection.prototype.find = function(selector, fields, options) {
	...
	if(sqlTable) {
		...
		for(var di in tableDesc.details) {
			...
			var detSql = "SELECT * FROM "+detTableName+" WHERE "+OracleDB.q("_id")+" IN (SELECT "+OracleDB.q("_id")+" FROM ("+sql0+"))";
...
OracleCollection.prototype.ensureIndex = function(fields, options, callback) {
	...
	if(cl["."]) {
		...
		if(s) {
		var sql = "CREATE "+(options.bitmap?"BITMAP ":"")+(options.unique?"UNIQUE ":"")+"INDEX "+OracleDB.q(sqlTable+"I1")+" ON "+OracleDB.q(sqlTable)+"("+s+")";
		var sqlParameters = [];
