...
OracleDB.prototype._getJsonColumns = function(sqlTable, op, oracleOptions) {
	var self = this;
	...
	for(var c in columns) {
		...
		if(op === 'u') {
			s = s + 'IF NOT(:NEW."'+c+'" = :OLD."'+c+'" OR (:NEW."'+c+'" IS NULL AND :OLD."'+c+'" IS NULL)) THEN ';
		...
		if(cv.isBoolean) {
			s = s + '\'"'+c+'": \'||nvl(:NEW."'+c+'", \'null\')';
		} else if(cv.dataType === 'VARCHAR2') {
			s = s + '\'"'+c+'": "\'||replace(replace(:NEW."'+c+'", chr(10), \'\\n\'), \'"\', \'\\"\')||\'"\'';
		} else if(cv.dataType === 'NUMBER') {
			s = s + '\'"'+c+'": \'||nvl(to_char(:NEW."'+c+'"), \'null\')';			
		} else if(cv.dataType === 'DATE') {
			s = s + '\'"'+c+'": {"$date": "\'||to_char(:NEW."'+c+'", \'yyyy-mm-dd"T"hh24:mi:ss".000Z"\')||\'"}\'';		
		} else if(cv.dataType === 'TIMESTAMP') {
			s = s + '\'"'+c+'": {"$date": "\'||to_char(:NEW."'+c+'", \'yyyy-mm-dd"T"hh24:mi:ss.ff3"Z"\')||\'"}\'';		
		}
...
OracleDB.prototype._refreshTable = function(sqlTable, oracleOptions) {
	  ...
	  trgSql = trgSql + "\t\to := '';\n";
	  trgSql = trgSql + self._getJsonColumns(sqlTable, 'i', oracleOptions) + "\n";
	  trgSql = trgSql + "\t\to := '{'||o||'}';\n";
	  trgSql = trgSql + "\t\to2 := null;\n";
	  trgSql = trgSql + '\t\tinsert into "oplog" ("id", "ts", "v", "op", "ns", "o", "o2")\n'
	  trgSql = trgSql + '\t\tvalues ("oplog_seq".nextval, current_timestamp, 2, op, ns, o, o2);\n';	  
	  trgSql = trgSql + "\tELSIF UPDATING THEN\n";
	  trgSql = trgSql + "\t\top := 'u';\n";
	  trgSql = trgSql + "\t\tns := 'meteor@localhost/XE'||'.'||'"+sqlTable+"';\n";
	  var cl = self._getJsonColumns(sqlTable, 'u', oracleOptions);
	  if(cl) {
	  trgSql = trgSql + "\t\to := '';\n"
	  trgSql = trgSql + cl + "\n";
	  ...
	  if(nested) {
		  trgSql = trgSql + "\t\t\to2 := '{\"_id\": \"'||:OLD.\"_id\"||'\", \"_indexNo\": '||:OLD.\"_indexNo\"||'}';\n";		  
	  } else {
		  trgSql = trgSql + "\t\t\to2 := '{\"_id\": \"'||:OLD.\"_id\"||'\"}';\n";
	  }
	  trgSql = trgSql + '\t\t\tinsert into "oplog" ("id", "ts", "v", "op", "ns", "o", "o2")\n';
	  trgSql = trgSql + '\t\t\tvalues ("oplog_seq".nextval, current_timestamp, 2, op, ns, o, o2);\n';
	  ...
	  trgSql = trgSql + "\t\tns := 'meteor@localhost/XE'||'.'||'"+sqlTable+"';\n";
	  if(nested) {
		  trgSql = trgSql + "\t\to := '{\"_id\": \"'||:OLD.\"_id\"||'\", \"_indexNo\": '||:OLD.\"_indexNo\"||'}';\n";		  
	  } else {
		  trgSql = trgSql + "\t\to := '{\"_id\": \"'||:OLD.\"_id\"||'\"}';\n";		  
	  }
	  trgSql = trgSql + "\t\to2 := null;\n";	  
	  trgSql = trgSql + '\t\tinsert into "oplog" ("id", "ts", "v", "op", "ns", "o", "o2")\n';
	  trgSql = trgSql + '\t\tvalues ("oplog_seq".nextval, current_timestamp, 2, op, ns, o, o2);\n';
