...
OracleDB.prototype._ensureTable = function(sqlTable, oracleOptions, parentSqlTable) {
	  ...
	  if(!self._tables[sqlTable]) {
		  ...
		  if(!self._tables[sqlTable]) {
    		  ...
    		  // Add a primary key
	    	  if(parentSqlTable) {
	    		  sql = "alter table "+OracleDB.q(sqlTable)+" add constraint "+OracleDB.q(sqlTable+"Pk")+" primary key ("+OracleDB.q("_id")+", "+OracleDB.q("_indexNo")+")";
	    	  } else {
		  		  sql = "alter table "+OracleDB.q(sqlTable)+" add constraint "+OracleDB.q(sqlTable+"Pk")+" primary key ("+OracleDB.q("_id")+")";
	    	  }
	  		  if(oracleOptions.sqlDebug) {
	  			  console.log(sql);
	  		  };
    		  ...
    		  // Add a foreign key
	    	  if(parentSqlTable) {
	    		  sql = "alter table "+OracleDB.q(sqlTable)+" add constraint "+OracleDB.q(sqlTable+"Fk")+" foreign key ("+OracleDB.q("_id")+") references "+OracleDB.q(parentSqlTable)+"("+OracleDB.q("_id")+") on delete cascade";
	    		  if(oracleOptions.sqlDebug) {
	    			  console.log(sql);
	    		  };
	    		  result = self.executeCommand(sql, []);
		  ...
		  if(!columnDesc) {
			  ...
			  if(value !== null && (value instanceof Date || !(value instanceof Array) && typeof value !== 'object')) {
				  if(columnDesc.isNull) {
					  ...
					  var sql = "alter table "+OracleDB.q(sqlTable)+" drop constraint "+ OracleDB.q(sqlTable + "NullChk" + columnDesc.columnId);
