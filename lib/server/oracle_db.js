...
OracleDB.prototype._refreshTable = function(sqlTable, oracleOptions) {
	  ...
	  if(!tableResult) {
		  var tableSql = 'select table_name as "tableName", status, num_rows as "numRows" from user_tables where table_name = upper(:1)';
	  ...
	  var colSql = 'select column_name as "columnName", column_id as "columnId", data_type as "dataType", data_length as "dataLength", data_precision as "dataPrecision", data_scale as "dataScale", nullable as "nullable", default_length as "defaultLength" from user_tab_columns where table_name = upper(:1) order by column_id';
	  ...
	  for(var i = 0; i < colResult.records.length; i++) {
		  ...
		  var column = rec.columnName;
		  var meteorColumn = column;
		  column = '"'+column+'"';
		  columns[column] = rec;
		  rec["meteorName"] = meteorColumn;
		  ccn[meteorColumn] = column;
	  }
	  tableResult["colRecords"] = colResult.records;
	  tableResult["columns"] = columns;
	  tableResult["ccn"] = ccn;
	  
	  var consSql = 'select constraint_name as "constraintName", constraint_type as "constraintType" from user_constraints where table_name = upper(:1) and status = \'ENABLED\' and invalid is null';
	  ...
	  for(var i = 0; i < consResult.records.length; i++) {
		  ...
		  var s = sqlTable.toUpperCase()+"_"+"BOOL_CHK_";
		  ...
		  s = sqlTable.toUpperCase()+"_"+"NULL_CHK_";
		  if(rec.constraintName.substr(0, s.length) == s) {
			  // It's null check!
			  var index = rec.constraintName.substr(s.length);
	  ...
	  var indSql = 'select index_name as "indexName", index_type as "indexType", uniqueness as "uniqueness", compression as "compression" from user_indexes where table_name = upper(:1)';
	  ...
	  var detSql = 
		  'select table_name as "tableName", table_name as "fieldName" from user_constraints where constraint_type=\'R\' ' +
...
OracleDB.prototype._getColumnType = function(sqlTable, column, value, oracleOptions, postponedBooleanColumns, postponedNullColumns) {
	  ...
	  if(type) {
		  if(value === null) {
			  ...
			  var sql = "select id from "+sqlTable+" where rownum = 1";
...
OracleDB.prototype._ensureColumnsPostProcess = function(sqlTable, oracleOptions, refreshCols, postponedBooleanColumns, postponedNullColumns) {
	  ...
	  for(var i = 0; i < postponedBooleanColumns.length; i++) {
		  ...
		  var sql = "alter table "+sqlTable+" add constraint "+ sqlTable + "_bool_chk_" + columnDesc.columnId + " CHECK (\""+
		  	columnDesc.columnName+"\" IN (NULL, '"+oracleOptions.booleanTrueValue+"', '"+oracleOptions.booleanFalseValue+"')) ENABLE";
	  ...
	  for(var i = 0; i < postponedNullColumns.length; i++) {
		  ...
		  var columnDesc = table.columns[column];
		  var sql = "alter table "+sqlTable+" add constraint "+ sqlTable + "_null_chk_" + columnDesc.columnId + " CHECK (\""+
		  	columnDesc.columnName+"\" IS NULL) ENABLE";
	  ...
	  for(var column in fields) {
		  ...
		  if(value === null) {
			  ...
			  if(!columnDesc) {
				  ...
				  var sql = "alter table "+sqlTable+" add ("+column+" "+type+")";
...
OracleDB.prototype._ensureColumns = function(sqlTable, doc, oracleOptions, parentSqlTable, doXCheck) {
		  ...
		  if(!columnDesc) {
			  ...
			  if(value !== null && (value instanceof Date || !(value instanceof Array) && typeof value !== 'object')) {
				  if(columnDesc.isNull) {
					  ...
					  var sql = "alter table "+sqlTable+" drop constraint "+ sqlTable + "_null_chk_" + columnDesc.columnId;
					  ...
					  // Check if there are any records in the table
					  sql = "select id from "+sqlTable+" where rownum = 1";
						  
					  result = self.executeCommand(sql, [], oracleOptions);

					  if(result.records.length === 0) {
						 // No rows, turn the column not null
						 sql = "alter table "+sqlTable+" modify ("+column+" not null)";
			  ...
			  if(value === null) {
				  if(columnDesc.nullable === 'N') {
					  // Allow NULLs
					  var sql = "alter table "+sqlTable+" modify ("+column+" null)";
				  ...
				  if(columnDesc.dataType === "VARCHAR2" && !columnDesc.isBoolean) {
					  ...
					  if(columnDesc.dataLength < len) {
						  var sql = "alter table "+sqlTable+" modify ("+column+" varchar2("+len+"))";
				  ...
				  if(columnDesc.dataType === "VARCHAR2") {
					  ...
					  if(newPrecision > precision || newScale > scale) {
						  var sql = "alter table "+sqlTable+" modify ("+column+" number("+newPrecision+", "+newScale+"))";
				  ...
				  if(columnDesc.isNull) {
					  ...
					  if(typeof oracleOptions.booleanTrueValue === 'number'
				  	  ...
				  	  if(type) {
				  		  var sql = "alter table "+sqlTable+" modify ("+column+" "+type+")";
	  ...
	  for(var i = 0; i < table.colRecords.length; i++) {
		  ...
		  if(colRec.columnName === "ID" || colRec.columnName === "INDEX_NO") {
			  continue;
		  }
		  ...
		  if(value === undefined || (value instanceof Array) || (value != null && typeof value === 'object')) {
			  ...
			  if(colRec.nullable === 'N') {
				  // Allow NULLs
				  var sql = "alter table "+sqlTable+" modify (\""+colRec.columnName+"\" null)";
