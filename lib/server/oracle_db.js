...
OracleDB.prototype._refreshTable = function(sqlTable, oracleOptions) {
	  ...
	  for(var i = 0; i < colResult.records.length; i++) {
		  ...
		  var column = oracleOptions.sqlConvertColumnNames ? rec.columnName.toLowerCase() : rec.COLUMN_NAME.toLowerCase();
		  columns[column] = rec;
		  
		  var meteorColumn;
		  if(oracleOptions.sqlConvertColumnNames) {
			  meteorColumn = OracleDB._columnNameO2M(column);
		  } else {
			  meteorColumn = column;
		  }
	  ...
	  var consSql = "select constraint_name, constraint_type from user_constraints where table_name = upper(:1) and status = 'ENABLED' and invalid is null";
	  ...
	  var indSql = "select index_name, index_type, uniqueness, compression from user_indexes where table_name = upper(:1)";
...
OracleDB.prototype._getColumnType = function(sqlTable, column, value, oracleOptions, postponedBooleanColumns, postponedNullColumns) {
	  ...
	  if(value === null) {
		  ...
		  var len = value.length > 0 ? value.length : 1;
		  type = "varchar2("+4000+")";
			  ...
			  if(value === null) {
				  ...
				  if(columnDesc.dataType === "VARCHAR2" && !columnDesc.isBoolean) {
					  ...
					  if(columnDesc.dataLength < len) {
						  var sql = "alter table "+sqlTable+" modify ("+column+" varchar2("+4000+"))";
