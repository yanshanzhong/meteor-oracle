...
OracleDB.prototype._refreshTable = function(sqlTable, oracleOptions) {
	  ...
	  var indSql = 'select index_name as "indexName", index_type as "indexType", uniqueness as "uniqueness", compression as "compression" from user_indexes where table_name = :1';
	  ...
	  var detSql = 
		  ...
		  "(select constraint_name from user_constraints where constraint_type in ('P','U') and table_name=:0) ";
...
OracleDB.prototype._ensureTable = function(sqlTable, oracleOptions, parentSqlTable) {
	  ...
	  if(!self._tables[sqlTable]) {
		  ...
		  if(!self._tables[sqlTable]) {
	    	  ...
	    	  if(parentSqlTable) {
	    		  sql = "create table \""+sqlTable+"\" (\"_id\" varchar2(17) not null, \"_indexNo\" number(20, 0) not null)";
	    	  } else {
	    		  sql = "create table \""+sqlTable+"\" (\"_id\" varchar2(17) not null)";	    		  
	    	  }
    		  ...
    		  // Add a primary key
	    	  if(parentSqlTable) {
	    		  sql = "alter table \""+sqlTable+"\" add constraint \""+sqlTable+"Pk\""+" primary key (\"_id\", \"_indexNo\")";
	    	  } else {
		  		  sql = "alter table \""+sqlTable+"\" add constraint \""+sqlTable+"Pk\""+" primary key (\"_id\")";
	    	  }
	  		  if(oracleOptions.sqlDebug) {
	  			  console.log(sql);
	  		  };
    		  ...
    		  // Add a foreign key
	    	  if(parentSqlTable) {
	    		  sql = "alter table \""+sqlTable+"\" add constraint \""+sqlTable+"Fk\""+" foreign key (\"_id\") references \""+parentSqlTable+"\"(\"_id\") on delete cascade";
	    		  if(oracleOptions.sqlDebug) {
	    			  console.log(sql);
	    		  };
	    		  result = self.executeCommand(sql, []);
...
OracleDB.prototype._getColumnType = function(sqlTable, column, value, oracleOptions, postponedBooleanColumns, postponedNullColumns) {
	  ...
	  if(type) {
		  if(value === null) {
			  ...
			  var sql = "select \"_id\" from \""+sqlTable+"\" where rownum = 1";
...
OracleDB.prototype._ensureColumnsPostProcess = function(sqlTable, oracleOptions, refreshCols, postponedBooleanColumns, postponedNullColumns) {
	  ...
	  for(var i = 0; i < postponedBooleanColumns.length; i++) {
		  ...
		  var sql = "alter table \""+sqlTable+"\" add constraint \""+ sqlTable + "BoolChk" + columnDesc.columnId + "\" CHECK (\""+
	  ...
	  for(var i = 0; i < postponedNullColumns.length; i++) {
		  ...
		  var columnDesc = table.columns[column];
		  var sql = "alter table \""+sqlTable+"\" add constraint \""+ sqlTable + "NullChk" + columnDesc.columnId + "\" CHECK (\""+
		  ...
		  if(!columnDesc) {
			  ...
			  if(value !== null && (value instanceof Date || !(value instanceof Array) && typeof value !== 'object')) {
				  if(columnDesc.isNull) {
					  ...
					  var sql = "alter table \""+sqlTable+"\" drop constraint \""+ sqlTable + "NullChk" + columnDesc.columnId + '"';
					  ...
					  // Check if there are any records in the table
					  sql = "select \"_id\" from \""+sqlTable+"\" where rownum = 1";
