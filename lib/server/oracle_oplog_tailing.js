...
_.extend(OplogHandle.prototype, {
  ...
  _tailingJob : function() {
	  ...
	  if(self._lastProcessedTS === null) {
	    // Find the last oplog entry.
	    var lastOplogEntry = self._oplogLastEntryConnection.findOne(
	      OPLOG_COLLECTION, {}, {sort: {ts: -1}, fields: {ts: 1}});

	    if (lastOplogEntry) {
	      // If there are any calls to callWhenProcessedLatest before any other
	      // oplog entries show up, allow callWhenProcessedLatest to call its
	      // callback immediately.
	      self._setLastProcessedTS(lastOplogEntry.ts);
	    }
	  }
	  ...
	  if(self._lastProcessedTS !== null) {
		  ...
		  oplogSelector.ts = {$gt: self._lastProcessedTS};

		  var tail = self._oplogTailConnection.find(
		  	      OPLOG_COLLECTION, oplogSelector, {skip: 0, limit: 100, sort: {ts: 1}, fields: {_id: 1, ts: 1, v:1, op:1, ns:1, o:1 , o2:1}});
		  ...
		  for(var i = 0; i < rows.length; i++) {
			  var row = rows[i];
		    	...
		    	row.ts = MongoInternals.NpmModule.Timestamp(row.ts % 1000000000, Math.floor(row.ts / 1000000000));
			  if(row.o) {
			  ...
			  console.log("TAILING JOB PUSHING: ", self._dbName, row);
			  
		        self._entryQueue.push(row);
		        self._maybeStartWorker();
		        
		        self._lastProcessedTS = row.ts;
		  }
	  }
  ...
  _startTailing: function () {
    ...
    self._oplogTailConnection = new OracleConnection();

    // XXX better docs, but: it's to get monotonic results
    // XXX is it safe to say "if there's an in flight query, just use its
    //     results"? I don't think so but should consider that
    self._oplogLastEntryConnection = new OracleConnection();

    self._tailingJobIntervalId = Meteor.setInterval(function f() {self._tailingJob()}, 200);
