...
_.extend(OracleSelector.prototype, {
  ...
  _compileSelector: function (selector) {
    ...
    if (LocalCollection._selectorIsId(selector)) {
      ...
      return "\"_id\" = "+_prepareOperand(selector);
    }
    ...
    if (key.substr(0, 1) === '$') {
      ...
      if(valueMatcher && valueMatcher !== "") {
          ...
          if(parts.length <= 1) {
        	  ...
        	  s = "\"_id\" IN (SELECT DISTINCT \"_id\" FROM \""+firstPartTable+"\" WHERE "+valueMatcher(column)+")";
              ...
              column = '"'+column+'"';
         	  ...
         	  s = "\"_id\" IN (SELECT DISTINCT \"_id\" FROM \""+firstPartTable+"\" WHERE \"+indexNo\" = "+parts[1]+" AND "+valueMatcher(column)+")";
          } else {
...
ELEMENT_OPERATORS = {
  ...
  $elemMatch: {
    ...
    compileElementSelector: function (operand, valueSelector, matcher, oracleSelector) {
    	return function (key) {
    		return "\"_id\" IN (SELECT DISTINCT \"_id\" FROM "+key+" WHERE "+
