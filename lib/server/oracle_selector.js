...
OracleSelector = function (selector, tableName) {
  var self = this;
...
OracleSelector._compilePrepared = function(selector, tableName) {
	var rs = {};
	for(fn in selector) {
		var value = selector[fn];
		
		if(fn === ".") {
			var s = new OracleSelector(value, tableName);
			rs[fn] =  s._docMatcher;
			rs["*"] = s._fields;
		} else {
			var nestedTableName = tableName+"$"+fn;
			
			rs[fn] =  OracleSelector._compilePrepared(value, nestedTableName);
		}
	}
...
OracleSelector.process = function(selector, tableName) {
	var s = OracleSelector._prepare(selector);

	s = OracleSelector._compilePrepared(s, tableName);
