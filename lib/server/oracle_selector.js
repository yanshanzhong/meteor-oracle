    ...
    if (key.substr(0, 1) === '$') {
      ...
      if(valueMatcher && valueMatcher !== "") {
          ...
          if(parts.length <= 1) {
        	  ...
        	  s = "id IN (SELECT DISTINCT id FROM "+firstPart+" WHERE "+valueMatcher(parts[1])+")";
         	  ...
         	  s = "id IN (SELECT DISTINCT id FROM "+firstPart+" WHERE index_no = "+parts[1]+" AND "+valueMatcher(parts[2])+")";
          } else {
