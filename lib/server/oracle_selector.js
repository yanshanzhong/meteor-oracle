...
OracleSelector._prepare = function(selector) {
	var sls = {};
...
OracleSelector._compile = function(selector) {
	var s = new OracleSelector(selector);

	return s._docMatcher;
};

OracleSelector._compilePrepared = function(selector) {
	var rs = {};
	for(fn in selector) {
		var value = selector[fn];
		
		if(fn === ".") {
			var s = new OracleSelector(value);
			rs[fn] =  s._docMatcher;
			rs["*"] = s._fields;
		} else {
			rs[fn] =  OracleSelector._compilePrepared(value);
		}
	}
...
OracleSelector.process = function(selector) {
	var s = OracleSelector._prepare(selector);

	s = OracleSelector._compilePrepared(s);
...
var _prepareOperand = function(operand) {
	  var ret = null;
	    if (typeof operand === 'boolean') {
	    	...
	    	ret = "to_date('"+_convertDate(operand)+"', 'mm/dd/yyyy hh24:mi:ss')";
	    } else {
	    	ret = operand;
	    }
    ...
    if (key.substr(0, 1) === '$') {
      ...
      if(valueMatcher && valueMatcher !== "") {
          ...
          if(parts.length <= 1) {
        	  ...
        	  s = OracleDB.q("_id")+" IN (SELECT DISTINCT "+OracleDB.q("_id")+" FROM "+OracleDB.q(firstPartTable)+" WHERE "+valueMatcher(column)+")";
              ...
              column = OracleDB.q(column);
         	  ...
         	  s = OracleDB.q("_id")+" IN (SELECT DISTINCT "+OracleDB.q("_id")+" FROM "+OracleDB.q(firstPartTable)+" WHERE "+OracleDB.q("_indexNo")+" = "+parts[1]+" AND "+valueMatcher(column)+")";
          } else {
    ...
    if (_.has(VALUE_OPERATORS, operator)) {
      ...
      operatorMatchers.push(
          options.compileElementSelector(
            operand, valueSelector, matcher, self));
    } else {
...
ELEMENT_OPERATORS = {
  ...
  $elemMatch: {
    dontExpandLeafArrays: true,
    compileElementSelector: function (operand, valueSelector, matcher, oracleSelector) {
    	return function (key) {
    		return OracleDB.q("_id")+" IN (SELECT DISTINCT "+OracleDB.q("_id")+" FROM "+key+" WHERE "+
