    ...
    if (key.substr(0, 1) === '$') {
      ...
      if(valueMatcher && valueMatcher !== "") {
          ...
          if(parts.length <= 1) {
        	  ...
        	  s = "id IN (SELECT DISTINCT id FROM "+firstPartTable+" WHERE "+valueMatcher(column)+")";
              ...
              var column = OracleDB._columnNameM2O(parts[2]);
         	  ...
         	  s = "id IN (SELECT DISTINCT id FROM "+firstPartTable+" WHERE index_no = "+parts[1]+" AND "+valueMatcher(column)+")";
          } else {
...
ELEMENT_OPERATORS = {
  ...
  $elemMatch: {
    ...
    compileElementSelector: function (operand, valueSelector, matcher, oracleSelector) {
    	return function (key) {
    		return "id IN (SELECT DISTINCT id FROM "+key+" WHERE "+
