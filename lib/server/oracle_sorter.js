...
OracleSorter._compilePrepared = function(sort) {
	var rs = {};
	for(fn in sort) {
		var value = sort[fn];
		
		rs[fn] = fn === "." ? OracleSorter._compile(value) : OracleSorter._compilePrepared(value);
	}
	
	return rs;
};

OracleSorter.process = function(sort) {
	var s = OracleSorter._prepare(sort);

	s = OracleSorter._compilePrepared(s);
