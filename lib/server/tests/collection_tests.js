...
Tinytest.add(
		  ...
		  function (test) {
		        ...
		        var rows = testLists.findOne({incompleteCount:22, ".testTags": {tag: {$like: '%A%'}}}, {skip: 0, limit: 10, sort: {ID: 1}, fields: {incompleteCount: true, userId: true, testTags: true, "testTags.tag": true, "testTags.owner": true}});
		        
		        console.log(rows);

		        rows = testLists.findOne({}, {skip: 0, limit: 10, sort: {ID: 1}});
		        
		        console.log(rows);

		        rows = testLists.findOne({'testTags.0.tag': "CANCELLED"}, {skip: 0, limit: 10, sort: {ID: 1}});
		        
		        console.log(rows);
		        
		        rows = testLists.findOne({"testTags": {$elemMatch: {$or: [{tag: "OK", owner: "Helen"}, {tag: "CANCELLED"}]}}}, {skip: 0, limit: 10, sort: {ID: 1}});
