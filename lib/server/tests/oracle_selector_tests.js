...
Tinytest.add('test oracle selector _compile', function (test, done) {
	...
	s = OracleSelector._compile({"tags.0.tag": {$eq: "OK"}});
	test.equal(s, "id IN (SELECT DISTINCT id FROM tags WHERE index_no = 0 AND tag = 'OK')");
	
	s = OracleSelector._compile({"tags": {$elemMatch: {tag: "OK", owner: "Helen"}}});
	test.equal(s, "id IN (SELECT DISTINCT id FROM tags WHERE (tag = 'OK') AND (owner = 'Helen'))");
...
Tinytest.add('test oracle selector process', function (test, done) {
	...
	test.equal(s, {".":"(name = 'Bill') AND (id IN (SELECT DISTINCT id FROM tags WHERE index_no = 0 AND tag = 'OK'))","*":{"name":null},"tags":{".":"owner = 'Helen'","*":{"owner":null}}});
