OracleModifier = function (modifier) {
	var self = this;
...
var MODIFIERS = {
  ...
  $addToSet: function (oracleModifier, field, arg) {
	  console.log("DEBUG $addToSet", oracleModifier, field, arg);
	  // TODO: fix this feature, eliminate the workaround
	  return OracleDB.q("_id")+" = "+OracleDB.q("_id");
...
OracleModifier._compilePrepared = function(modifier) {
	...
	for(fn in modifier) {
		...
		if(fn === ".") {
			var m = new OracleModifier(value);
			
			rs[fn] =  m._setClause;
			rs["*"] = m._fields;
			rs["$"] = m._parameters;
		} else {
			rs[fn] = OracleModifier._compilePrepared(value);
		}
	}
